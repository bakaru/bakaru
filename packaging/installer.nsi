...
;Parse package.json
!ifdef WIN_PATHS
    !searchparse /file "..\package.json" '"version": "' BAKARU_VERSION '",'
!else
    !searchparse /file "../package.json" '"version": "' BAKARU_VERSION '",'
!endif
!searchreplace BAKARU_VERSION_CLEAN "${BAKARU_VERSION}" "-" ".0"
!ifdef WIN_PATHS
...
!define COMPANY_NAME "Alexander Kukhta"
!define PRODUCT_VERSION "${BAKARU_VERSION}"
!define APP_NAME "Bakaru"


Name "${APP_NAME}"
Caption "${APP_NAME} ${BAKARU_VERSION}"
BrandingText "${APP_NAME} ${BAKARU_VERSION}"
VIAddVersionKey "ProductName" "${APP_NAME}"
VIAddVersionKey "ProductVersion" "${BAKARU_VERSION}"
VIAddVersionKey "FileDescription" "${APP_NAME} ${BAKARU_VERSION} Installer"
VIAddVersionKey "FileVersion" "${BAKARU_VERSION}"
VIAddVersionKey "CompanyName" "${COMPANY_NAME}"
VIAddVersionKey "LegalCopyright" "${APP_URL}"
VIProductVersion "${BAKARU_VERSION_CLEAN}.0"
!ifdef WIN_PATHS
    OutFile "..\build\artifacts\${APP_NAME}-${BAKARU_VERSION}-Setup.exe"
!else
    OutFile "../build/artifacts/${APP_NAME}-${BAKARU_VERSION}-Setup.exe"
!endif
...
;Define UI settings
!ifdef WIN_PATHS
    !define MUI_UI_HEADERIMAGE_RIGHT "..\icon.ico"
...
!define MUI_FINISHPAGE_RUN "$INSTDIR\Bakaru.exe"
...
Section
    ...
    ;Add the files
    !ifdef WIN_PATHS
        File /r "..\build\portable\Bakaru-win32-ia32\"
    !endif
...
Section
    ...
    CreateShortCut "$SMPROGRAMS\${APP_NAME}\${APP_NAME}.lnk" "$INSTDIR\Bakaru.exe" "" "$INSTDIR\resources\app\icon.ico" "" "" "" "${APP_NAME} ${BAKARU_VERSION}"
    CreateShortCut "$SMPROGRAMS\${APP_NAME}\Uninstall ${APP_NAME}.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\resources\app\icon.ico" "" "" "" "Uninstall ${APP_NAME}"
    ...
    WriteRegStr HKCU "${UNINSTALL_KEY}" "DisplayVersion" "${BAKARU_VERSION}"
    WriteRegStr HKCU "${UNINSTALL_KEY}" "DisplayIcon" "$INSTDIR\resources\app\icon.ico"
    WriteRegStr HKCU "${UNINSTALL_KEY}" "Publisher" "${COMPANY_NAME}"
    WriteRegStr HKCU "${UNINSTALL_KEY}" "UninstallString" "$INSTDIR\Uninstall.exe"
    WriteRegStr HKCU "${UNINSTALL_KEY}" "InstallString" "$INSTDIR"
    WriteRegStr HKCU "${UNINSTALL_KEY}" "URLInfoAbout" "${APP_URL}"
    WriteRegStr HKCU "${UNINSTALL_KEY}" "HelpLink" "https://gitter.im/bakaru/bakaru"
...
Function finishpageaction
    CreateShortCut "$DESKTOP\${APP_NAME}.lnk" "$INSTDIR\Bakaru.exe" "" "$INSTDIR\resources\app\icon.ico" "" "" "" "${APP_NAME} ${BAKARU_VERSION}"
FunctionEnd